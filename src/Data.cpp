...
namespace spruce
{
  ...
  Data::Data() : num_rows(0), num_rows_rounded(0), num_cols(0), snp_data(0), num_cols_no_snp(0), externalData(true), index_data(0), max_num_unique_values(
                                                                                                                                        0),
                 order_snps(false), any_na(false)
  {
  }
                                    ...
                                    std::vector<std::string> &dependent_variable_names)
  {
    ...
    while (getline(input_file, line))
    {
      ...
      {
        throw std::runtime_error(
            std::string("Could not open input file. Too few columns in row ") + std::to_string(row) + std::string(". Are all values numeric?"));
      }
      ++row;
    }
    num_rows = row;
