...
void run_spruce(const ArgumentHandler &arg_handler, std::ostream &verbose_out)
{
  verbose_out << "Starting Spruce." << std::endl;
  ...
  forest->writeOutput();
  verbose_out << "Finished Spruce." << std::endl;
}
...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    {
      run_spruce(arg_handler, std::cout);
    }
    else
    {
      ...
      run_spruce(arg_handler, logfile);
    }
  }
  catch (std::exception &e)
  {
    std::cerr << "Error: " << e.what() << " Spruce will EXIT now." << std::endl;
    return -1;
  }
