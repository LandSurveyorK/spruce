...
namespace spruce
{
  ...
  ArgumentHandler::ArgumentHandler(int argc, char **argv) : caseweights(""), depvarname(""), fraction(0), holdout(false), memmode(MEM_DOUBLE), savemem(false), skipoob(false), predict(
                                                                                                                                                                                   ""),
                                                            predictiontype(DEFAULT_PREDICTIONTYPE), randomsplits(DEFAULT_NUM_RANDOM_SPLITS), splitweights(""), tau(DEFAULT_POISSON_TAU), nthreads(
                                                                                                                                                                                             DEFAULT_NUM_THREADS),
                                                            predall(false), alpha(DEFAULT_ALPHA), minprop(DEFAULT_MINPROP), maxdepth(
                                                                                                                                DEFAULT_MAXDEPTH),
                                                            file(""), impmeasure(DEFAULT_IMPORTANCE_MODE), targetpartitionsize(0), minbucket(0), mtry(0), outprefix(
                                                                                                                                                              "spruce_out"),
                                                            probability(false), splitrule(DEFAULT_SPLITRULE), statusvarname(""), ntree(DEFAULT_NUM_TREE), replace(
                                                                                                                                                              true),
                                                            verbose(false), write(false), treetype(TREE_CLASSIFICATION), seed(0), usedepth(false)
  {
    this->argc = argc;
    this->argv = argv;
  }
  ...
  void ArgumentHandler::checkArguments()
  {
    ...
    // Check splitrule
    if (((splitrule == AUC || splitrule == AUC_IGNORE_TIES) && treetype != TREE_SURVIVAL) || (splitrule == MAXSTAT && (treetype != TREE_SURVIVAL && treetype != TREE_REGRESSION)) || (splitrule == BETA && treetype != TREE_REGRESSION) || (splitrule == HELLINGER && treetype != TREE_CLASSIFICATION && treetype != TREE_PROBABILITY) || (splitrule == POISSON && treetype != TREE_REGRESSION))
    {
      throw std::runtime_error("Illegal splitrule selected. See '--help' for details.");
    }
  ...
  {
    std::cout << "Ranger version: " << SPRUCE_VERSION << std::endl;
    std::cout << std::endl;
    std::cout << "Please cite Ranger: " << std::endl;
    std::cout
        << "Wright, M. N. & Ziegler, A. (2017). spruce: A Fast Implementation of Random Forests for High Dimensional Data in C++ and R. Journal of Statistical Software 77:1-17."
        << std::endl;
    std::cout << std::endl;
    std::cout << "BibTeX:" << std::endl;
    std::cout << "@Article{," << std::endl;
    std::cout
        << "    title = {{spruce}: A Fast Implementation of Random Forests for High Dimensional Data in {C++} and {R},"
        << std::endl;
